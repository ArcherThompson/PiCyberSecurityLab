...
# Check if story.markup exists in the current directory
if [[ -f "world_building/story.markup" ]]; then
    echo "Found story.markup. Copying to story data directory..."
    cp world_building/story.markup /var/www/html/story_data/story.markup
else
    echo "Error: story.markup not found in the current directory. Please add the file and re-run this script."
    exit 1
fi

# Copy the Python script
cat <<'EOF' > /var/www/html/story_data/process_markup.py
import sqlite3
import os
...
# Ensure proper permissions for the Python script
sudo chmod +x /var/www/html/story_data/process_markup.py

# Activate the virtual environment and run the Python script
echo "Processing the markup file with the Python script..."
source /var/www/html/story_data/venv/bin/activate
python /var/www/html/story_data/process_markup.py
deactivate
